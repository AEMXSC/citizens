...
function buildScene7Rendition(src, { width, format }) {
  ...
  // Some Scene7 renditions bake the dimensions into the PATH (e.g.
  // `.../Rebrand Hero_FamilyCouch-w2000-h857-fpng-alpha`). Those pre-sized
  // assets reject ALL query parameters — appending `?wid=`/`?fmt=` returns
  // HTTP 403. Detect the `-w<N>-h<N>` suffix and serve the path verbatim
  // (no query string) so the image loads. Only base template paths (no
  // baked size) accept the resizing/format params below.
  if (/-w\d+-h\d+/i.test(base)) {
    return base;
  }
...
// on Scene7 IS/Image or DM Open API URLs preserve their query parameters
// instead of having them stripped by the path-only optimizer in aem.js.
 ...
 */
/**
 * Icons live in the code repo (served by the EDS delivery host), not the AEM
 * author. On the Universal Editor author canvas the page is served from the
 * author origin, so relative /icons/*.svg paths 404. Rebase SVG icon images to
 * the EDS code host during authoring so they render. Published site is untouched.
 */
const EDS_CODE_HOST = 'https://main--nyl--aemxsc.aem.page';
function rebaseIcons(scope) {
  if (!window.location.hostname.endsWith('.adobeaemcloud.com')) return;
  (scope || document).querySelectorAll('img[src*="/icons/"]').forEach((img) => {
    try {
      ...
      img.src = `${EDS_CODE_HOST}${pathname}`;
    } catch (e) { /* ignore */ }
  });
}
...
async function loadLazy(doc) {
  ...
  rebaseIcons(main);
  window.setTimeout(() => rebaseIcons(main), 1500);
